...
Game_map::Game_map() {
  ...
  victory = {
      "========================================================================"
      "=================================",
      "*           Congratulations the mighty warrior you have managed to "
...
void Game_map::print_base() {
  for (auto &line : map) {
    for (auto &c : line) {
      std::cout << rang::style::dim << rang::bg::black << rang::fgB::green
                << rang::style::reset << get_color_for(c) << c
                << rang::style::reset;
    }
    std::cout << '\n';
  }
...
void Game_map::print_victory(bool *end_game) {
  ...
  for (auto &line : victory) {
    for (auto &c : line) {
      std::cout << rang::style::blink << rang::bg::green << rang::fgB::black
                << rang::style::reset << get_color_for(c) << c
                << rang::style::reset;
    }
    std::cout << '\n';
...
// function that will provide me with size of Map for placing objects in random
// places
...
// Function that generates random numbers that can be used after verification to
// randomly place monsters, users and possibly in future other objects on the
// map
...
// function that allow to find location of the player on the map, variable i
// allows to find row and result refers to column
std::vector<Game_map::Coordinates> Game_map::find_all(
    char player_type) { // keep an eye on return type it have Game_map:: !!!!!
  index_of_all = {};
  index_of_all.clear();
  size_t x = 0;
  size_t y = 0;
...
Game_map::Coordinates Game_map::next_position(Coordinates from,
                                              Move_direction direction) {

  // using coord to perform one search to get value for x & y, this is better
  // then: size_t x = find_way().x; size_t y = find_way().y; in this line I am
  // preforming search twice
  auto x = from.x;
  auto y = from.y;
...
void Game_map::move_player(Move_direction direction, int steps, bool *end_game,
  ...
  while (steps-- > 0) {
    ...
    my_Player.player_moves(); // this function decreases value of variable moves
                              // type int inside player and is passed as
                              // reference - not sure if I need that at all
...
void Game_map::move_monster() { // todo change way that monster is placed
                                // randomly on the map and moved from there
...
std::ostream &operator<<(std::ostream &out,
                         const Game_map::Coordinates &coordinates) {
